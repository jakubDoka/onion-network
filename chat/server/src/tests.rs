use {
    ...
    libp2p::futures::future::{join_all, try_join},
};
...
async fn message_block_finalization() {
    ...
    match jakub.invite_member(chat, kubo.keys.name, chat_spec::Member::best()).await {
        Err(e)
            if let Some(ChatError::AlreadyExists) = e.root_cause().downcast_ref::<ChatError>() => {}
        e => {
            e.unwrap();
            kubo.read_mail().await.unwrap();
            kubo.flush_vault().await.unwrap();
        }
    }
    ...
    let mem = kubo
        ...
        .await
        .unwrap()
        .fetch_my_member(kubo.keys.identity())
        .await
        .unwrap();
    kubo.vault.borrow_mut().chats.get_mut(&chat).unwrap().action_no = mem.action + 1;
