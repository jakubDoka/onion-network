...
const MAX_UNFINALIZED_BLOCKS: usize = 3;
...
fn select_finalizer(members: &[U256], selector: crypto::Hash) -> PeerId {
...
pub async fn handle_message_block(
    ...
    crate::ensure!(select_finalizer(&group, chat.selector) == origin, InvalidBlock(NotExpected));

    let proposed = unpack_messages_ref(block).map(crypto::hash::from_slice).collect::<HashSet<_>>();

    let present_count = unpack_messages_ref(&chat.buffer)
        .filter(|msg| proposed.contains(&crypto::hash::from_slice(msg)))
        .count();

    if proposed.len() != present_count {
        log::warn!("proposed block has extra messages {:?} {:?}", proposed.len(), present_count);
        chat.next_selector();
    ...
    let kept_len =
        retain_messages(&mut chat.buffer, |msg| !proposed.contains(&crypto::hash::from_slice(msg)))
            .len();
    debug_assert_eq!(chat.buffer.len() - kept_len, block.len());
...
fn reconstruct_chat(block_data: &mut ReplVec<(PeerId, MinimalChatData)>) -> MinimalChatData {
    ...
    let selector = {
        let mut selectors =
            block_data.iter().map(|(_, data)| data.selector).collect::<ReplVec<_>>();
        selectors.sort_unstable();
        selectors.chunk_by(|a, b| a == b).max_by_key(|chunk| chunk.len()).unwrap()[0]
    };

    let members = retain_members(block_data);
    let number = block_data.first().map(|(_, data)| data.number).unwrap_or(0);
    MinimalChatData { number, members, selector }
}
...
impl Chat {
    ...
    pub async fn resolve_finalization(
        ...
        while self.buffer.len() >= FINALIZATION_TRIGGER_SIZE {
            let selected = select_finalizer(group, self.selector);
